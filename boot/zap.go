...
import (
	"fmt"
	"github.com/flipped-aurora/gva/boot/internal"
	library "github.com/flipped-aurora/gva/library/directory"
...
func (z *_zap) Initialize() {
	if ok, _ := library.Directory.PathExists(global.Config.Zap.Director); !ok { // 判断是否有Director文件夹
		fmt.Println(`创建文件夹成功, 文件夹名称为: `, global.Config.Zap.Director)
	...
	if z.level == zap.DebugLevel || z.level == zap.ErrorLevel {
		z.zap = zap.New(internal.Zap.GetEncoderCore(z.writer, z.level), zap.AddStacktrace(z.level))
	} else {
		z.zap = zap.New(internal.Zap.GetEncoderCore(z.writer, z.level))
	}
