...
func (a *api) Init() error {
	entities := []model.Api{
		...
		{Model: global.Model{ID: 81, CreatedAt: time.Now(), UpdatedAt: time.Now()}, Path: "/api/deleteApisByIds", Method: "批量删除api", ApiGroup: "api", Description: "DELETE"},
	}
	return global.Db.Transaction(func(tx *gorm.DB) error {
		if tx.Where("id IN ?", []int{1, 81}).Find(&[]model.Api{}).RowsAffected == 2 {
			color.Danger.Printf("\n[postgres] --> apis 表的初始数据已存在!")
			return nil
		}
